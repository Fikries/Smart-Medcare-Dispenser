...
// Capture the status from the URL, default to 'all'
$status = isset($_GET['status']) ? $_GET['status'] : 'all';

// Prepare SQL query based on status
if ($status === 'successful') {
    $displaymotorspinsql = $conn->prepare("SELECT `id`, `eldername`, `medicinename`, `medicinetype`, `datetime`, `spinstate` FROM `motorspin` WHERE `spinstate` = 'true'");
    ...
    $displaymotorspinsql = $conn->prepare("SELECT `id`, `eldername`, `medicinename`, `medicinetype`, `datetime`, `spinstate` FROM `motorspin`");
}
...
<body>
    ...
    <h1>Medcare Medication Successful Record</h1>
        ...
        <table id="motorTableBody">
            ...
            <tbody>
                ...
                while ($displaymotorspinsql->fetch()) {
                    $statusColor = ($statespinmotor === 'true') ? 'green' : 'red'; // Color for successful or unsuccessful
                ?>
                    <tr>
                        ...
                        <td style="background-color: <?php echo $statusColor; ?>;"><?php echo ($statespinmotor === 'true') ? 'Successful' : 'Unsuccessful'; ?></td>
                    </tr>
                <?php } ?>
            </tbody>
        </table>
